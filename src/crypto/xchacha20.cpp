...
void xchacha20::discard(size_t length) {
	...
	if(length % sizeof(keystream)) {
		update();
		pos = std::uint8_t(length % sizeof(keystream));
	}
...
void xchacha20::crypt(const char * in, char * out, size_t length) {
	...
	for(size_t i = 0; i < length; i++, pos++) {
		...
		std::uint8_t key = std::uint8_t(keystream[pos / sizeof(word)] >> ((pos % sizeof(word)) * 8));
		out[i] = char(std::uint8_t(in[i]) ^ key);
	}
...
void xchacha20::increment_count(word state[16], size_t increment) {
	...
	std::uint64_t count = (std::uint64_t(state[13]) << (sizeof(word) * 8)) + state[12];
	count += increment;
	state[12] = word(count);
